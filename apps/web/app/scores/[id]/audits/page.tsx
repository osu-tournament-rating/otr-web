...
export default async function ScoreAuditsPage({
  params,
}: {
  params: PageParams;
}) {
  ...
  const scoreData = entityState.data as { game?: { matchId?: number } } | null;
  const matchId = scoreData?.game?.matchId;
  const backHref = matchId ? `/matches/${matchId}` : '/audits';
  const backLabel = matchId ? 'Back to Match' : 'Back to Audit Logs';

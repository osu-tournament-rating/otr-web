...
describe('admin data mutation freeze window', () => {
  it('is active on Tuesdays from 11:45 UTC through 12:14 UTC', () => {
    ...
    expect(
      isAdminDataMutationFreezeWindow(new Date('2026-06-02T12:14:59.999Z'))
    ).toBe(true);
  });

  it('is inactive outside the Tuesday UTC window', () => {
    expect(
      isAdminDataMutationFreezeWindow(new Date('2026-06-02T11:44:59.999Z'))
    ).toBe(false);
    expect(
      isAdminDataMutationFreezeWindow(new Date('2026-06-02T12:15:00.000Z'))
    ).toBe(false);
    expect(
      isAdminDataMutationFreezeWindow(new Date('2026-06-03T11:45:00.000Z'))
    ).toBe(false);
  });

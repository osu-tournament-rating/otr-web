import {
  ...
  MatchWarningFlags,
  Ruleset, TournamentProcessingStatus, TournamentRejectionReason,
  VerificationStatus
...
export const TournamentProcessingStatusMetadata: {
  ...
  [TournamentProcessingStatus.NeedsApproval]: {
    text: "Needs Approval",
    description: "Tournament is awaiting approval from a verifier before processing begins"
  },
...
export function formatMatchRejectionReasons(value: MatchRejectionReason) {
  return getEnumFlags(value, MatchRejectionReason).map(flag => MatchRejectionReasonMetadata[flag]);
}

/** Formats a {@link MatchRejectionReason} into displayable text */
export function formatMatchWarningFlags(value: MatchWarningFlags) {
  return getEnumFlags(value, MatchWarningFlags).map(flag => MatchWarningFlagMetadata[flag]);
}